document.addEventListener('DOMContentLoaded', () => {
  ...
  // 添加鼠标悬停效果
  if (sunContainer && sunCore) {
    sunContainer.addEventListener('mouseenter', () => {
      sunCore.style.boxShadow = '0 0 40px rgba(249, 245, 110, 1)';
      sunCore.style.transform = 'translate(-50%, -50%) scale(1.1)';
      sunCore.style.transition = 'all 0.5s ease';
    });
    
    sunContainer.addEventListener('mouseleave', () => {
      sunCore.style.boxShadow = '0 0 20px rgba(249, 245, 110, 0.8)';
    ...
    setInterval(() => {
      ...
      const randomBrightness = 0.8 + Math.random() * 0.2;
      
      sunCore.style.transform = `translate(-50%, -50%) scale(${randomScale})`;
      sunCore.style.boxShadow = `0 0 20px rgba(249, 245, 110, ${randomBrightness})`;
